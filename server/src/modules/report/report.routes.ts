...
import { createReportSchema, updateReportSchema, createSectionSchema, createTableSchema } from './report.validation';
import {
  createReport,
  getReports,
  getReportById,
...
// Report routes
router.route('/')
  .post(requireAuth, requireRole(['super_admin', 'employee']), validate(createReportSchema), createReport)
  .get(requireAuth, getReports);

router.route('/:id')
  .get(requireAuth, getReportById)
  .put(requireAuth, requireRole(['super_admin', 'employee']), validate(updateReportSchema), updateReport)
...
router.route('/:reportId/sections')
  .post(requireAuth, requireRole(['super_admin', 'employee']), validate(createSectionSchema), createSection);
...
router.route('/sections/:sectionId/tables')
  .post(requireAuth, requireRole(['super_admin', 'employee']), validate(createTableSchema), createTable);
